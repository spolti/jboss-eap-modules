        ...
        for i in ${LOGGER_CATEGORIES}; do
            ...
            logger_level=$(echo $logger | awk -F':' '{print $2}')
             if [[ ! "${allowed_log_levels[@]}" =~ " ${logger_level}" ]]; then
                log_info "Log Level ${logger_level} is not allowed, ${allowed_log_levels[@]}"
             else
                log_info "Configuring logger category ${logger_category} with level ${logger_level}"
                logger="<logger category=\"${logger_category}\">\n                \
<level name=\"${logger_level}\"/>\n\            \
</logger>\n            \
<!-- ##LOGGER-CATEGORY## -->"
